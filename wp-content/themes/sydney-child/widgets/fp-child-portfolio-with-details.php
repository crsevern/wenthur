...
class Sydney_Child_Portfolio_With_Details extends WP_Widget
{

    public function __construct()
    {
        $widget_ops = array('classname' => 'sydney_portfolio_widget', 'description' => __('Display your projects with details in a list.', 'sydney'));
        parent::__construct(false, $name = __('Sydney Child FP: Portfolio With Details', 'sydney'), $widget_ops);
        $this->alt_option_name = 'sydney_child_portfolio_with_details_widget';
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $output .= '<div class="projects-container" data-portfolio-effect="fadeInUp">';
        ...
        while ($the_query->have_posts()) :
            ...
            if (has_post_thumbnail()) {
                ...
                if ($project_url) :
                    $output .= '<div class="project-item item ' . $termsString . '">';
                    $output .= '<a class="project-pop-wrap" target="_blank" href="' . esc_url($project_url) . '">';
                    $output .= '<div class="project-pop"></div>';
                    $output .= ($show_project_title == 1) ? $project_title : '';
                    $output .= '</a>';
                    $output .= '<a class="thumbnail-container" target="_blank" href="' . esc_url($project_url) . '">';
                    $output .= '<div style="background-image: url(' . get_the_post_thumbnail_url($post->ID) . ');"></div>';
                    $output .= '</a>';
                    $output .= '<div class="project-content">' . $content . '</div>';
                    $output .= '</div>';
                else :
                    $output .= '<div class="project-item item ' . $termsString . '">';
                    ...
                    $output .= '<div class="project-pop"></div>';
                    $output .= ($show_project_title == 1) ? $project_title : '';
                    $output .= '</a><a class="thumbnail-container" href="' . get_the_permalink() . '">';
                    $output .= '<div style="background-image: url(' . get_the_post_thumbnail_url($post->ID) . ');"></div>';
                    $output .= '</a>';
                    $output .= '<div class="project-content">' . $content . '</div>';
                    $output .= '</div>';
